# Delete all GitHub packages for a user
# Usage: .\delete-all-packages.ps1
...
try {
    ...
    foreach ($package in $packages) {
        ...
        $versionsUrl = "https://api.github.com/users/$Username/packages/$PackageType/$encodedPackageName/versions"
        ...
        if ($versions.Count -eq 0) {
            Write-Host "  No versions found for this package" -ForegroundColor Gray
            continue
        }
        
        foreach ($version in $versions) {
            $versionId = $version.id
            $versionName = if ($version.name) { $version.name } else { "ID: $versionId" }
            
            if ($WhatIf) {
                Write-Host "  [WHATIF] Would delete version: $versionName" -ForegroundColor Magenta
            } else {
                Write-Host "  Deleting version: $versionName" -ForegroundColor Red
                try {
                    $deleteUrl = "https://api.github.com/users/$Username/packages/$PackageType/$encodedPackageName/versions/$versionId"
